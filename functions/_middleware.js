...
const API_INFO_SECTION = `<section class="api-info-panel" id="api-info" aria-labelledby="api-info-title"><div class="section-head compact"><p class="eyebrow">API Data</p><h2 id="api-info-title">API로 확인할 수 있는 정보</h2><p>기상청 API Hub 자료를 Cloudflare Function에서 JSON으로 바꿔 화면에 표시합니다. 인증키가 없을 때는 예시 자료로 화면 구성을 확인할 수 있습니다.</p></div><div class="api-health" id="apiHealthStatus">API 연결 상태를 확인하는 중입니다.</div><div class="api-info-grid"><article><span>국내 영향 태풍</span><h3>SfcYearlyInfoService/getTyphoonList</h3><p>연도별 태풍 중 국내 영향 여부를 상륙, 직접영향, 간접영향, 영향 없음으로 분류합니다.</p><dl><div><dt>확인 항목</dt><dd>태풍번호, 한글명, 영문명, 발생기간, 최저기압, 최대풍속, 국내 영향 분류</dd></div><div><dt>현재 응답</dt><dd id="koreaApiInfo">대기 중</dd></div></dl></article><article><span>연도별 태풍목록</span><h3>typ_lst.php</h3><p>특정 연도에 발생한 태풍 목록과 한반도 영향 여부를 조회합니다.</p><dl><div><dt>확인 항목</dt><dd>태풍번호, 진행상태, 영향 분류, 시작·종료 시각, 이름, 설명</dd></div><div><dt>현재 응답</dt><dd id="listApiInfo">대기 중</dd></div></dl></article><article><span>태풍정보 + 예측</span><h3>typ_now.php / typ_data.php</h3><p>현재 분석 위치와 발표번호별 예측 경로를 지도와 표에 표시합니다.</p><dl><div><dt>확인 항목</dt><dd>분석·예측시각, 위도·경도, 이동방향, 이동속도, 중심기압, 최대풍속, 강풍반경, 확률반경</dd></div><div><dt>현재 응답</dt><dd id="forecastApiInfo">대기 중</dd></div></dl></article></div></section>`;
