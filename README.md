...
- `/private-rental`: 청약홈 민간임대·공공지원 민간임대 공고와 주택형별 접수건수·경쟁률 조회
...
민간임대 청약은 [한국부동산원 청약홈 분양정보 조회 서비스](https://www.data.go.kr/tcs/dss/selectApiDataDetailView.do?publicDataPk=15098547)의 민간임대와 공공지원 민간임대 자료를 별도 화면에서 조회합니다. 각 공고의 주택관리번호와 공고번호로 [청약접수 경쟁률 및 특별공급 신청현황 조회 서비스](https://www.data.go.kr/tcs/dss/selectApiDataDetailView.do?publicDataPk=15098905)를 지연 호출해 주택형·공급유형별 공급세대수, 접수건수와 공식 경쟁률을 표시합니다. 접수 상태는 공식 접수 시작일과 종료일을 기준으로 표시하며 실제 자격, 임대조건과 계약은 청약홈 최신 공고문을 확인해야 합니다.
...
- `APPLYHOME_API_KEY`: 선택적인 청약홈 민간임대 전용 인증키이며 없으면 `DATA_GO_KR_API_KEY` 또는 `LH_API_KEY` 사용
- `APPLYHOME_COMPETITION_API_KEY`: 선택적인 청약홈 경쟁률 전용 인증키이며 없으면 `APPLYHOME_API_KEY`, `DATA_GO_KR_API_KEY` 또는 `LH_API_KEY` 사용
