...
test("the home is a compact support landing with twelve latest posts", async () => {
  ...
  assert.match(html, /<h1 id="home-title">필요한 지원금을 빠르게 찾으세요<\/h1>/);
  assert.match(html, /정부지원금 빠른 확인/);
  assert.match(html, /공식 자료를 기준으로 신청 대상, 기간, 서류, 신청처를 한곳에 정리했습니다\./);
  ...
  assert.match(html, /class="home-keywords"/);
  assert.match(html, /K%ED%8C%A8%EC%8A%A4#support-search/);
  assert.match(html, /청년월세/);
  assert.match(html, /class="home-finder-stats"/);
  ...
  assert.match(html, /\/blog\.css\?v=20260828-home4/);
  ...
  assert.match(css, /\.home-finder\s*\{[\s\S]*?display:\s*grid;[\s\S]*?grid-template-columns:\s*minmax\(0, 1fr\) minmax\(300px, 0\.68fr\);[\s\S]*?margin-top:\s*26px/);
  assert.match(css, /\.home-finder h1\s*\{[\s\S]*?font-size:\s*clamp\(2rem, 4\.2vw, 2\.7rem\);[\s\S]*?letter-spacing:\s*0/);
  assert.match(css, /\.home-search-row\s*\{[\s\S]*?display:\s*flex;[\s\S]*?gap:\s*8px/);
  assert.match(css, /\.home-search button\s*\{[\s\S]*?background:\s*#1d4ed8;[\s\S]*?color:\s*#fff/);
  assert.match(css, /\.home-keywords\s*\{[\s\S]*?display:\s*flex;[\s\S]*?flex-wrap:\s*wrap/);
  assert.match(css, /\.home-finder-stats\s*\{[\s\S]*?display:\s*flex;[\s\S]*?flex-wrap:\s*wrap/);
  ...
  assert.match(css, /@media \(max-width: 767px\)[\s\S]*?\.home-finder\s*\{[\s\S]*?padding:\s*22px/);
